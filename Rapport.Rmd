...
meanCd_IC <- clean.Cd[,list(moyCd=mean(Cd), 
...
v.cross <- variogram(g, cutoff=100, width=50)
...
Cd.vario <- variogram(g, cutoff=1, width=0,1)
...
v.cross <- variogram(g, cutoff=1, width=20)
```
...
ggplot() + 
  ...
  geom_point(data=na.omit(clean.data3), 
             aes(x=X, y=Y, shape="Cadmium data", size="Cadmium data"),
             color="white") +
  scale_shape_manual("", values=c(1,8)) +
  scale_size_manual("", values=c(4,2)) +
  labs(fill="Cd predictions [mg/l]") +
  theme(legend.key=element_rect(fill="#34668c", 
                                color=NA)) +
  xlab("Longitude") +
  ylab("Latitude") +
