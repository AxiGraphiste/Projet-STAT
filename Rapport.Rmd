...
KEEEEEEEEEEP### Semi-variogram pour Cd
...
KEEEEEEEEEEEP### Semi-variogram pour Pb
...
KEEEEEEEEP### Semi-variogram pour Cr
...
###affichage de la grille pour Cd

<<<<<<< Updated upstream
=======
Cd.sim <- krige(formula = Cd~1, data = data3, loc=~X+Y, newdata = data3.grid,
                 model = fit.vg.model, nsim = 1000, nmax = 20)
...
###affichage de la grille pour Pb
...
###affichage de la grille pour Cr
...
Cd.idw <- idw(formula = Cdbx~1, data = na.omit(data3), locations = ~x+y, newdata = na.omit(data3.grid), idp = 2, nmax=20, maxdist=20)

setnames(Cd.idw, "var1.pred", "Cd.pred")

# We create a function to plot results to avoid copy pasting many lines of code. 
...
plot.predictions(Cd.idw, "Cadmium concentration in the Province of Liege - Inverse Distance prediction")
mean(Cd.idw$Cd.pred)
...
```{r cross vario}
g <- gstat(id="Cd", formula=Cd~1, data=na.omit(clean.data3), locations=~X+Y)
...
```{r}
#créer la grid de prédiction
gridsize = 0.5
x <- seq(floor(min(clean.data3$X)), # from minimum longitude
         ceiling(max(clean.data3$X)), # to maximum longitude
         by=gridsize)
y <- seq(floor(min(clean.data3$Y)), # from minimum latitude
         ceiling(max(clean.data3$Y)), # to maximum latitude
         by=gridsize)
dyle.grid <- as.data.table(expand.grid(x=x, y=y)) # create a data.table containing all grid points
```
...
#Code
Cd.gstat <- gstat(formula = Cdbx~1, data = clean.Cd, locations = ~X+Y)
Cd.vario <- variogram(Cd.gstat, cutoff =5000 , width = 798)
vg.model <- vgm(psill = 2, model ='Exp' , range= 600)
fit.vg.model <- fit.variogram(Cd.vario, model=vg.model)
plot(Cd.vario, model=fit.vg.model, main="Variogram of Cd", pch=16, col="black")
#Variance 
...
vg.model <- vgm(psill = 2, model ='Exp' , range= 600)
fit.vg.model <- fit.variogram(Pb.vario, model=vg.model)
plot(Pb.vario, model=fit.vg.model, main="Semi-variogram of Pb", pch=16, col="black")
#Variance 
trellis.focus("panel",1,1)
llines(x=c(195331, 279509.0), y=c(93704, 163250.0), col="red", lwd=1, lty=2)
trellis.unfocus()
...
vg.model <- vgm(psill = 100, model ='Exp' , range= 1200)
fit.vg.model <- fit.variogram(Cr.vario, model=vg.model)
plot(Cr.vario, model=fit.vg.model, main="Semi-variogram of Cr", pch=16, col="black")
#Variance 
trellis.focus("panel",1,1)
llines(x=c(195331, 291500.0), y=c(93704, 163250.0), col="red", lwd=1, lty=2)
trellis.unfocus()
